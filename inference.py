...
def run_episode(client: OpenAI, env: FakeNewsEnvironment, task: str) -> float:
    ...
    while not obs.done and step_count < initial_budget + 2:
        try:
            ...
            step_count += 1
            print(f"[STEP] step={step_count} reward={obs.reward if obs.reward is not None else 0.01:.4f}", flush=True)
            break
    ...
    final_score = obs.reward if obs.reward is not None else 0.01
...
def main():
    ...
    for task in ["easy", "medium", "hard"]:
        ...
        all_results[task] = {
            ...
            "min_score": round(min(scores), 4) if scores else 0.01,
            "max_score": round(max(scores), 4) if scores else 0.01,
            "episodes": len(scores),
...
def run_heuristic_fallback():
    ...
    for task in ["easy", "medium", "hard"]:
        ...
        for _ in range(5):
            ...
            ep_score = obs.reward if obs.reward is not None else 0.01
